...
class ProvinceController extends Controller {
    ...
    public function datatable(Request $request)
    {
        ...
        $query_data = Province::where(function($query) use ($search) {
                if($search) {
        ...
        $total_filtered = Province::where(function($query) use ($search) {
                if($search) {
                    $query->where(function($query) use ($search) {
                        $query->where('name', 'like', "%$search%")
                            ->orWhere('latitude', 'like', "%$search%")
        ...
        if($validation->fails()) {
            ...
            if($query) {
                activity()
                    ...
                    ->log('Menambah data provinsi ' . $query->name);
    ...
    public function update(Request $request, $id)
    {
        ...
        if($validation->fails()) {
            ...
            $query = Province::where('id', $id)->update([
                'name'      => $request->name,
                'latitude'  => $request->latitude,
                'longitude' => $request->longitude
            ]);
    ...
    {
        $query = Province::where('id', $request->id)->delete();
        if($query) {
            activity()
                ->performedOn(new Province())
                ->causedBy(session('id'))
