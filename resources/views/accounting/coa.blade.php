        ...
        <div class="row layout-top-spacing" id="cancel-row">
            <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
                <div class="widget-content widget-content-area br-6">
                    <div class="row mb-5">
                        <div class="col-md-6">
                            <h5 class="mt-2">Daftar Data COA</h5>
                        </div>
                        <div class="col-md-6 text-right">
                            <button type="button" class="btn btn-info" data-toggle="modal" data-target="#form_modal"
                                id="add_data" onclick="cancel()"><svg xmlns="http://www.w3.org/2000/svg" width="24"
                ...
                <div class="modal-body">
                    ...
                    <form id="form_data">
                        ...
                        <div class="form-group mb-0">
                            <label>Parent :</label>
                            <select name="parent_id" id="parent_id" class="select2 form-control" onchange="getLatestSub()"></select>
                        </div>
                        <div class="form-group">
                            <label>Deskripsi :</label>
                            <textarea name="description" id="description" class="form-control"
                                placeholder="Masukan deskripsi" style="resize:none;"></textarea>
                        </div>
                        <div class="form-group">
                            <label>Status :</label>
    ...
    <script>
        $(function() {
            ...
            var table = $('#datatable_serverside').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: '{{ url("accounting/coa/datatable") }}',
                    type: 'POST',
                    headers: {
            ...
            $('#datatable_serverside tbody').on('click', 'td.details-control', function() {
                ...
                if(row.child.isShown()) {
                    ...
                    row.child(subcoa(row.data())).show();
                    tr.addClass('shown');
        ...
        function subcoa(data) {
            var tbody = '';

            $.ajax({
                url: '{{ url("accounting/coa/get_sub") }}',
                type: 'POST',
                async: false,
                data: {
                    code: data[2]
                ...
                success: function(response) {
                    if(response.length > 0) {
                        ...
                        tbody += `
                            <tr class="text-center">
                                <td class="align-middle" colspan="5">Tidak ada sub</td>
                            </tr>
                        `;
                    }
                },
        ...
        function getParent() {
            $.ajax({
                url: '{{ url("accounting/coa/get_parent") }}',
                type: 'POST',
                dataType: 'JSON',
                cache: false,
                headers: {
        ...
        function getLatestSub() {
            $.ajax({
                url: '{{ url("accounting/coa/get_latest_sub") }}',
                type: 'POST',
                async: false,
                data: {
                    id: $('#parent_id').val(), 
                ...
                success: function(response) {
                    if(response.parent_id == 0){
                        $number = parseInt(response.code);
                    }else{
                        $number = parseInt(response.code) +1;
                    }
                    $('#code').val($number);
                },
                error: function() {
        ...
        function update(id) {
            $.ajax({
                url: '{{ url("accounting/coa/update") }}' + '/' + id,
                type: 'POST',
                dataType: 'JSON',
                data: $('#form_data').serialize(),
                headers: {
        ...
        function destroy(id) {
            swal({
                ...
                if(result.value) {
                    $.ajax({
                        url: '{{ url("accounting/coa/destroy") }}',
                        type: 'POST',
                        dataType: 'JSON',
                        data: {
                            id: id
