                                        ...
                                        @foreach($m->sub as $s)
                                            <tr>
                                                ...
                                                <td class="align-middle text-center">
                                                    <input type="checkbox" name="menu_id[]" id="menu_id_{{ $s->id }}" value="{{ $s->id }}">
                                                </td>
                                            </tr>
                                        @endforeach
                                    @else
                                        <tr>
                                            ...
                                            <td class="align-middle text-center">
                                                <input type="checkbox" name="menu_id[]" id="menu_id_{{ $m->id }}" value="{{ $m->id }}">
                                            </td>
                                        </tr>
                                    @endif
                                @endforeach
    ...
    <script>
        ...
        function loadDataTable() {
            $('#datatable_serverside').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: '{{ url("setting/role/datatable") }}',
                    type: 'POST',
                    headers: {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(response.role_access, function(i, val) {
                        $('input[type="checkbox"][value="' + val.menu_id + '"]').prop('checked', true);
                    });
        ...
        function update(id) {
            $.ajax({
                url: '{{ url("setting/role/update") }}' + '/' + id,
                type: 'POST',
                dataType: 'JSON',
                data: new FormData($('#form_data')[0]),
                processData: false,
