        ...
        <div class="row layout-top-spacing" id="cancel-row">
            <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
                ...
                <div class="widget-content widget-content-area br-6">
                    <div class="row mb-5">
                        <div class="col-md-6">
                            <h5 class="mt-2">Klaim Pembelian <b>{{ $repayment->code }}</b></h5>
                        </div>
                        @if($repayment->claims)
                        <div class="col-md-6">
                            <div class="form-group text-right">
                                <a href="{{ url('download/pdf?param=claim&type=repayments&id=' . $repayment->id) }}"
                    ...
                    <form action="{{ url('purchase/claim/' . $repayment->id) }}" method="POST" id="form_data">
                        @csrf
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control form-control-plaintext"
                                        value="{{ $repayment->claims ? date('Y-m-d', strtotime($repayment->claims->date)) : date('Y-m-d') }}"
                                        readonly>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="description" id="description"
                                        ...
                                        value="{{ $repayment->claims ? $repayment->claims->description : '' }}" {{
                                        $repayment->claims ? 'disabled' : 'required' }}>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>Perhitungan :</label>
                                    <select name="flag" id="flag" class="form-control" required {{ $repayment->claims ?
                                        'disabled' : 'required' }}>
                                        <option value="1" {{ $repayment->claims ? $repayment->claims->flag == 1 ?
                                            'selected' : '' : '' }}>Berat</option>
                                        <option value="2" {{ $repayment->claims ? $repayment->claims->flag == 2 ?
                                            'selected' : '' : '' }}>Qty</option>
                                    </select>
                                </div>
                            </div>
				...
				if (this.options.dictRemoveFile) {
				  return Dropzone.confirm("Are You Sure to "+this.options.dictRemoveFile, function() {
					...
					$.ajax({
						...
						type: 'POST',
						url: '{{url("purchase/destroy_picture")}}',
						data: {filename: name},
						success: function (data){
